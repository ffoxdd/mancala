...
class DefaultPockets {
    static pockets() {
        return OrderedMap([
            ...DefaultPockets._side("A"),
            ...DefaultPockets._side("B"),
        ])
    }

    static _side(player) {
        return [
            ...DefaultPockets._sidePockets(player),
            DefaultPockets._mancala(player),
        ]
    }
...
class Board extends Record({pockets: DefaultPockets.pockets(), hand: 0}) {
    hasStonesInHand() {
        return this.hand > 0
    }
